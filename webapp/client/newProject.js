Template.newProjectModal.onRendered(function() {
  ...
  $createProjectForm = instance.$(".new-project.ui.form");
  ...
  $createProjectForm.form({
    fields: {
      projectLabel: {
        rules: [
          { type: "empty", prompt: "Please enter the project ID" }
        ],
      },
      projectDescription: {
        rules: [
          ...
          {
            type: "minLength[10]",
            prompt: "The project description must be at least 10 characters long",
          }
        ],
      }
    }
  ...
  instance.$('.create-project.ui.modal').modal({
    onApprove(clickedElement) {
      console.log("onApprove", clickedElement);
      if (clickedElement[0].className.indexOf("createProjectAndSearch") !== -1) {
        console.log("need to also do project search");
      }

      // if the form isn't valid, return false to not hide the modal
      if (!$createProjectForm.form("validate form")) {
        return false;
      }

      var form_vals = $createProjectForm.form("get values");
      ...
      Meteor.call("createProject", form_vals);
    }
  }); //.modal("show");
});
